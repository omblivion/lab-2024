    ...
    return (
        <Form className='block-example border border-primary rounded mt-4 mb-0 px-5 py-4 form-padding'
            ...
            <Form.Group className='mb-3'>
                ...
                <Form.Control className={errors.title ? 'wrong-field' : ''} type='text' required={true} value={title}
                              onChange={event => setTitle(event.target.value)}/>
            </Form.Group>

            <Form.Group className='mb-3'>
                <Form.Check type='checkbox' label='Favorite' name='favorite' checked={favorite}
            ...
            <Form.Group className='mb-3'>
                ...
                <Form.Control className={errors.date ? 'wrong-field' : ''} type='date' value={watchDate}
                              onChange={event => {
                                  event.target.value ? setWatchDate(dayjs(event.target.value).format('YYYY-MM-DD')) : setWatchDate('')
                              }}/>
            </Form.Group>
            ...
            <Form.Group className='mb-3'>
                ...
                <Form.Control className={errors.rating ? 'wrong-field' : ''} type="number" min={0} max={5} step={1}
                              value={rating}
                ...
                <div id="errors" className='pt-1 pb-2'>
                    { /* errors.map((error, index) => (<p  key={index}>{error}</p>)) } */
                        Object.keys(errors).map((err, index) => (
                            <p className='error-message' key={index}><b>{"Error " + (index + 1) + ": "}</b>{errors[err]}
                            </p>))}
                </div>
                : ""
            }
